...
const ICON_FOR_MODE: { [p in QuizMode]: { label: string, icon: ReactNode } } = {
	hearing: {
		icon:
			<Hear color={'#6b7280'} width={48} height={48} />,
		label: 'Słuchanie',
	},
	reading: {
		icon: <ReadBook color={'#6b7280'} width={48} height={48} />,
		label: 'Czytanie',
	},
		...
		<View className={`w-full h-auto pb-12 pt-4 px-4 gap-8`}>
			...
			<View className={'flex-row justify-around items-center'}>
				{config.availableModes.map((mode, index) =>
					<View key={index}>
						<TouchableOpacity className={'w-32 h-32'} onPress={() => selectMode(mode)}>
							<View className={'bg-gray-100 items-center justify-center rounded-2xl w-full h-full'}>
								{ICON_FOR_MODE[mode].icon}
							</View>

						</TouchableOpacity>
						<Text
							className={'mt-2 font-semibold text-lg text-center'}>{ICON_FOR_MODE[mode].label}</Text>
					</View>,
				)}
			</View>
		</View>
